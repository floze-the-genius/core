...
(function () {
  ...
  F.build = (view) => {
    ...
    setCoin(0, 0, 0);
    if (A.gl) {
      // the WebGL layer draws a real metal coin in place of the CSS one
      el.classList.add('gl3d');
      A.gl.flip.anchor(el.querySelector('.flip-scene'));
    }
  };
  ...
  F.play = (face, btn) => {
    ...
    if (A.gl) {
      A.gl.flip.dim(false);
      A.gl.flip.glow(false);
      A.gl.flip.toss(result, () => {
        A.sfx.clink();
        A.fx.shake(4, 0.15);
        U.after(0.3, () => {
  ...
  function resolve(result, face, amount, win) {
    ...
    if (win) {
      ...
      if (A.gl) {
        A.gl.flip.glow(true);
        U.after(0.5, () => A.gl.flip.glow(false));
        A.gl.burst(c.x, c.y, 10 + streak * 4, 0.8);
      }
      ...
      if (A.gl) A.gl.flip.dim(true);
      U.after(1.4, () => {
        if (busy) return;
        el.classList.remove('lost');
        if (A.gl) A.gl.flip.dim(false);
      });
      A.sfx.lose();
