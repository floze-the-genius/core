...
(function () {
  ...
  UI.init = () => {
    ...
    // title coin
    titleCoin = A.gl ? A.gl.coinAdapter($('#title-coin'), 'title') : new A.BigCoin($('#title-coin'), { autoSpin: true });
    titleCoin.setSkin(2);
  ...
  function buildCoinView(v) {
    ...
    const meter = () => ({ value: G.isFever() ? (G.s.feverUntil - G.s.time) / E.feverDur(G.s) : G.s.fever, active: G.isFever() });
    if (A.gl) {
      // 3D coin: the canvas stays as the hit area, the fever meter becomes an SVG ring
      const wrap = v.querySelector('.coin-wrap');
      wrap.insertAdjacentHTML(
        'beforeend',
        '<svg class="fever-ring" viewBox="0 0 100 100"><defs><linearGradient id="frg" x1="0" y1="1" x2="1" y2="0"><stop offset="0" stop-color="#ffd23f"/><stop offset=".5" stop-color="#ff7a1a"/><stop offset="1" stop-color="#ff2d55"/></linearGradient></defs><circle class="fr-bg" cx="50" cy="50" r="42.5"/><circle class="fr-fg" cx="50" cy="50" r="42.5"/></svg>'
      );
      bigCoin = A.gl.coinAdapter(wrap, 'main');
      const ring = wrap.querySelector('.fr-fg');
      const len = 2 * Math.PI * 42.5;
      ring.style.strokeDasharray = len;
      let shown = 0;
      bigCoin.frame = () => {
        const m = meter();
        shown = U.lerp(shown, m.value, 0.2);
        ring.style.strokeDashoffset = len * (1 - U.clamp(shown, 0, 1));
        wrap.classList.toggle('fever', m.active);
      };
    } else bigCoin = new A.BigCoin(cv, { meter });
  ...
  function clickCoin(x, y) {
    ...
    if (A.gl) A.gl.burst(x, y, r.crit ? 8 : fever ? 2 : 1, r.crit ? 1 : 0.6);
    if (flyingCoins < 14) {
      flyingCoins++;
  ...
  UI.setTab = (id, force) => {
    ...
    if (window.gsap && !force) {
      const kids = Array.from(views[id].children).filter((k) => !k.matches('canvas'));
      window.gsap.fromTo(kids, { y: 26, opacity: 0 }, { y: 0, opacity: 1, duration: 0.45, stagger: 0.06, ease: 'power3.out', clearProps: 'transform,opacity' });
    }
    requestAnimationFrame(() => {
      const g = A.games[id];
      if (g && g.show) g.show();
  ...
  function catchGolden() {
    ...
    if (r.kind === 'coins') {
      ...
      if (A.gl) A.gl.burst(c.x, c.y, 20, 1);
    } else {
  ...
  UI.frame = (dt) => {
    ...
    if (tab === 'coin') {
      ...
      if (A.gl) A.gl.vault.setLocks(G.s.locks);
      else if (G.atVault() && vaultCv) A.vault.drawDoor(vaultCv, { locks: G.s.locks, time: performance.now() / 1000 });
    }
