...
(function () {
  ...
  M.boot = async () => {
    A.i18n.set(A.settings.lang);
    A.fx.init();
    // Three.js arrives as an ES module; the game falls back to its 2D renderer without it.
    const libs = await A.loadThree();
    if (libs) {
      try {
        A.gl = null;
        A.gl = A.initGL(libs);
    ...
    if (window.gsap) {
      const letters = U.$$('.logo-text .ll');
      if (letters.length) window.gsap.from(letters, { y: -140, rotation: () => U.rand(-40, 40), opacity: 0, duration: 1.1, ease: 'bounce.out', stagger: 0.07, delay: 0.2 });
    }
  ...
  M.toTitle = () => {
    if (A.gl && G.s.won) A.gl.vault.reset();
    G.save();
    A.ui.show('title');
    A.fx.setTheme('title');
  ...
  function showEnd() {
    ...
    const st = s.stats;
    const items = [
      ['clock', s.time, U.fmtTime],
      ['tap', st.clicks, U.fmt],
      ['bolt', st.crits, U.fmt],
      ['chip', st.bets, U.fmt],
      ['trophy', st.bestWin, U.fmt],
      ['coin', s.total, U.fmt],
      ['clover', st.golden, U.fmt],
      ['crown', st.jackpots, U.fmt],
    ];
    $('#end-stats').innerHTML = items
      .map(([ic, v, f], i) => `<div class="stat" style="animation-delay:${0.4 + i * 0.08}s">${A.icon(ic)}<b>${f(v)}</b></div>`)
      .join('');
    if (window.gsap) {
      // numbers roll up from zero, one card after another
      U.$$('#end-stats .stat b').forEach((b, i) => {
        const [, v, f] = items[i];
        const o = { v: 0 };
        window.gsap.to(o, { v, duration: 1.6, delay: 0.5 + i * 0.12, ease: 'power3.out', onUpdate: () => (b.textContent = f(o.v)) });
      });
    }
    A.fx.setTheme('vault');
    A.ui.show('end');
