...
(function () {
  ...
  FX.setTheme = (name, instant) => {
    if (A.gl) A.gl.setTheme(name, instant || !theme.top);
  ...
  let pulse = 0;
  FX.pulse = (v) => {
    pulse = Math.min(1.5, pulse + (v || 0.3));
    if (A.gl) A.gl.pulse(v);
  };
  ...
  // canvas-confetti gives ribbons that flutter and tumble; the built-in version is the fallback.
  let confettiLib = null;
  function libConfetti() {
    if (confettiLib || !window.confetti) return confettiLib;
    const cv = document.createElement('canvas');
    cv.id = 'confetti';
    document.body.appendChild(cv);
    confettiLib = window.confetti.create(cv, { resize: true, useWorker: true });
    return confettiLib;
  }
  FX.confetti = (x, y, n, opts) => {
    opts = opts || {};
    const lib = libConfetti();
    if (lib) {
      const count = Math.ceil(n * FX.quality);
      const base = { colors: CONF, shapes: ['square', 'circle', 'star'], scalar: 1.1, ticks: 260, disableForReducedMotion: true };
      if (opts.wide)
        lib(Object.assign({}, base, { particleCount: count, angle: 270, spread: 160, startVelocity: 18, gravity: 0.7, origin: { x: 0.5, y: -0.15 }, drift: U.rand(-0.5, 0.5) }));
      else {
        const o = { x: x / Math.max(1, W), y: y / Math.max(1, H) };
        lib(Object.assign({}, base, { particleCount: Math.ceil(count * 0.6), angle: 60, spread: 70, startVelocity: 55, origin: o }));
        lib(Object.assign({}, base, { particleCount: Math.ceil(count * 0.6), angle: 120, spread: 70, startVelocity: 55, origin: o }));
      }
      return;
    }
    n = Math.ceil(n * FX.quality);
  ...
  FX.rain = (dur, rate) => {
    if (A.gl) A.gl.rain(dur, Math.max(2, (rate || 6) * 0.6));
    const t0 = U.now();
    const spawn = () => {
  ...
  FX.banner = (text, style, opts) => {
    ...
    if (gs) inner.innerHTML = Array.from(text, (ch) => `<span class="bl">${ch === ' ' ? '&nbsp;' : ch}</span>`).join('');
    ...
    if (gs) {
      b.classList.add('gs');
      const letters = inner.querySelectorAll('.bl');
      gs.from(letters, { yPercent: -120, rotationX: -100, scale: 1.8, opacity: 0, duration: 0.7, ease: 'back.out(2.2)', stagger: 0.045 });
      gs.from(b, { scale: 0.6, duration: 0.8, ease: 'elastic.out(1, 0.5)' });
      U.after(life, () => {
        gs.to(letters, {
          x: () => U.rand(-260, 260),
          y: () => U.rand(-220, 120),
          rotation: () => U.rand(-120, 120),
          opacity: 0,
          scale: 0.4,
          duration: 0.55,
          ease: 'power2.in',
        ...
        gs.to(b.querySelectorAll('.banner-sub, .rays'), { opacity: 0, scale: 1.3, duration: 0.4 });
        U.after(0.7, () => b.remove());
      });
      return b;
    }
    U.after(life, () => {
