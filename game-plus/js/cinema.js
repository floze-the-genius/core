...
(function () {
  ...
  // ---- finale in 3D: the UI steps aside and the real vault door opens ----
  function finale3D() {
    return new Promise((resolve) => {
      ...
      const W = window.innerWidth;
      const H = window.innerHeight;
      for (let i = 0; i < 14; i++) U.after(1.0 + i * 0.09, () => A.sfx.tick(0.6 + i * 0.03));
      U.after(2.25, () => {
        A.sfx.clunk(true);
        A.fx.shake(14, 0.6);
      });
      U.after(2.45, () => A.sfx.rumble(2.2));
      U.after(3.3, () => {
        A.sfx.choir();
        A.sfx.coinRain(4);
        A.fx.flash('#fff6d0', 0.7, 1.6);
        A.fx.shake(22, 1);
      });
      for (let i = 0; i < 12; i++) U.after(3.8 + i * 0.3, () => A.fx.firework(U.rand(W * 0.1, W * 0.9), U.rand(H * 0.1, H * 0.45)));
      U.after(4.2, () => A.fx.confetti(0, 0, 220, { wide: true, down: true }));
      U.after(6.4, () => {
        busy = false;
        resolve(() => document.body.classList.remove('finale'));
      });
    });
  }
  ...
  C.finale = () =>
    A.gl ? finale3D() : new Promise((resolve) => {
      busy = true;
